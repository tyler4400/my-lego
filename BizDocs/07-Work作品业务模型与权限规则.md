...
H5 成品页路由：`GET /{PREFIX}/{VERSION}/work/pages/:id/:uuid`

| 模式 | 触发方式 | 准入规则 |
|---|---|---|
| 正式访问 | 不带 query 或 `?preview=false` | 仅 `status=Published` |
| 预览模式 | `?preview=true`（也兼容 `?preview` / `?preview=1`） | 允许任何非 `Deleted` 状态（含 Initial） |

**预览模式的鉴权策略（设计决策）**：

- **不鉴权**：拿到 `(id, uuid)` 的任何人加上 `?preview=true` 都能看到草稿。
- 这是一个有意识的取舍。已知隐私风险：作品 id + uuid 一旦泄露，未发布草稿就可能被围观。
- 接受此风险的理由：
  1. `uuid` 是 `nanoid(8)` 随机串，不易爆破
  2. 预览二维码仅在编辑器内向作者本人展示，泄露面有限
  3. 加鉴权会破坏 "扫码即看" 的体验（作者要在手机端登录）
  4. 真的泄露了，作者可以重新生成（删旧建新）止损
- 软删除（`status=Deleted`）**依然不可预览**，避免"删了之后还能访问"的体验问题。

实现位置：`packages/craft-backend/src/module/work/workToH5.service.ts` 的 `getPageData(id, uuid, preview)`。
...
| 2 | **作品可以没有渠道**（`channels` 为 0~N 个） | 后端没有"至少 1 个"的限制；统计层面无 channel 参数的访问归入"默认"桶 |
| 3 | 渠道操作必须是作者本人 | CASL `ManageChannels`，admin 也允许 |
| 4 | 渠道号自动生成（`nanoid(6)`），**永远稳定**、不可修改 | 只能改 `name`；外部投放链接已印，id 一旦变化所有历史统计就断链 |

#### 5.3 前端业务行为（后端不感知，但实现时必须保证）

下面是**前端业务行为**，后端没有强制约束（也无须感知），但前端实现时务必保证，否则会破坏运营统计的连续性：

- **渠道号（id）永远不能变**：渠道增删改请直接调 `POST /work/channel/{create|update|delete}` 接口，**不要试图通过 `update` 把整个 `channels` 数组覆盖回去**。后者会导致 id 被前端重新生成，外部投放链接全部失效。
- **UX 鼓励但不强制保留渠道**：发布弹窗 UI 上，**剩最后一个渠道时隐藏该渠道的删除按钮**（避免误删后失去任何投放链接）。数据层允许为 0，但 UI 上不暴露这条路径。
- **不再"自动创建默认渠道"**：旧规则要求"初次发布时前端默认创建一个名为'默认'的渠道"，本期已取消。作品可以零渠道发布，访问不带 channel 参数的 URL 时由统计层归桶。
...
- **预览能力的鉴权强化**（已知缺口，本期接受）。当前预览不鉴权，未来如出现"草稿被围观"的实际投诉，可加 JWT 校验或短期签名 token，但会损失"扫码即看"体验，需权衡。
